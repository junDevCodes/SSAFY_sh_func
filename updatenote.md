...
## V7.7.1 (2026-01-26) - Hotfix: Python Path & Install Fix 🚑

### 🐛 버그 수정 (Critial Bug Fix)
- **`gitup` 실행 오류 해결**: 시스템에 `python3`만 존재하고 `python` 명령어가 없는 환경( Git Bash 등)에서 "Python was not found" 오류가 발생하던 문제를 수정했습니다.
  - 개선 전: `python` 명령어 고정 (일부 환경 호환 불가)
  - 개선 후: `python`, `python3` 등 사용 가능한 인터프리터를 **동적으로 감지**하여 실행 (`PYTHON_CMD`)
- **설치 스크립트(`install.sh`) 개선**: Windows Git Bash 등에서 `~/.bashrc` 대신 `~/.bash_profile`이나 `~/.profile`을 사용하는 경우, 설치 후 명령어를 인식하지 못하던 문제를 해결했습니다.
  - 이제 `~/.bash_profile` 및 `~/.profile`이 존재하면 해당 파일에도 설정(`source ...`)을 자동으로 추가합니다.
