# 알고리즘/실습 자동화 Shell 함수 (V8.2.6)

이 저장소는 Bash 함수들을 제공하여 알고리즘 풀이와 SSAFY 실습 과제 제출을 자동화합니다.
별도의 복잡한 설치 없이 스크립트 파일만 복사하여 즉시 사용할 수 있습니다.

## 📌 V8.2.6 주요 변경
- **`algo-update` 비대화형 환경 blocking 수정** (`lib/update.sh`): 재시작 프롬프트가 CI/테스트 환경에서 blocking되는 버그 수정 (`else` → `elif _is_interactive`)
- **`gitdown` 패널 flush 보정**: 단계별 패널 `begin/end` 짝 정상화 및 flush 순서 보정
- **`algo-update --force`**: 동일 버전에서도 강제 재설치 가능
- **백업 보존 정책**: swap 성공 후에만 과거 백업 정리, 실패 시 기존 백업 유지
- **`test_update_flow.sh` 6/6 PASS** 포함 전체 회귀 테스트 통과
